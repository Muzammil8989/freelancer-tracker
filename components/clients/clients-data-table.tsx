...
import { makeColumns, type Client } from './columns'
...
export function ClientsDataTable({
  data, totalCount, page, perPage, search, sort, order,
  ...
  // Build a new URL preserving all current params, merging updates
  const updateParams = useCallback(
    (updates: Record<string, string | number | undefined>) => {
      ...
      // Always reset to page 1 when anything other than page changes
      const qs = params.toString()
      startTransition(() => router.push(`${pathname}${qs ? `?${qs}` : ''}`))
    },
    [search, page, perPage, sort, order, pathname, router]
  )
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {totalCount === 0
            ? 'No results'
            : `Showing ${from}–${to} of ${totalCount.toLocaleString()} client${totalCount !== 1 ? 's' : ''}`}
        </p>
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm text-muted-foreground px-3 tabular-nums" aria-live="polite" aria-atomic="true">
            {page} / {totalPages}
          </span>
          <Button
            variant="outline" size="icon" className="h-8 w-8"
